...
package body System.Traceback is
   ...
   procedure Call_Chain
     (Traceback   : in out System.Traceback_Entries.Tracebacks_Array;
      ...
      Return_Address_Offset : constant System.Address :=
        System.Word_Size / 8;
      --  Offset in bytes where return address of current frame is stored,
      --  relative to current frame. The value is one word on x86 (4 bytes)
      --  and x86_64 (8 bytes) so it's convenient to use Word_Size here.
   ...
   begin
      ...
      --  The following "if" statement will generate a warning since
      --  Return_Address_Offset is a constant that is dependent on target
      --  architecture.
      if Return_Address_Offset = 8 then
         Asm ("mov %%rbp, %0",
